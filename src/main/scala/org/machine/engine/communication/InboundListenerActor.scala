...
class InboundListenerActor extends Actor{
  ...
  override def preStart(){
    zeroMQContext = ZMQ.context(zmq.ZMQ.ZMQ_IO_THREADS) //Constants at: https://github.com/zeromq/jeromq/blob/master/src/main/java/zmq/ZMQ.java
  ...
  private def processInboundMessages:Unit = {
    val inboundRequest = socket.recv(0) // TODO: Magic number...
