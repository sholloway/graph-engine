...
class FindDownStreamElementsByElementId(database: GraphDatabaseService,
  ...
  private def findElements(database: GraphDatabaseService, cmdOptions: GraphCommandOptions):List[Element] = {
    ...
    |return end.mid as elementId,
    |   labels(end) as elementTypes,
    |   keys(end) as keys,
    |   end.element_description as element_description,
    |   end.creation_time as creation_time,
    |   end.last_modified_time as last_modified_time,
    |   end as data
    """.stripMargin
