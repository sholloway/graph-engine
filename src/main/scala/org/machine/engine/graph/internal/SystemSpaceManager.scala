...
object SystemSpaceManager extends StrictLogging{
  ...
  def verifySystemSpace(db: GraphDatabaseService):SystemSpace = {
  ...
  private def insertNewSystemSpace(db: GraphDatabaseService,
    logger: Logger):SystemSpace = {
      ...
      if (systemSpaces.isEmpty){
        logger.error("Engine: System Space could not be created.")
